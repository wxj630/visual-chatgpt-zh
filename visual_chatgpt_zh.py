...
class ImageEditing:
    def __init__(self, device):
        ...
        self.inpainting = StableDiffusionInpaintPipeline.from_pretrained(f"{pretrained_model_dir}/stable-diffusion-inpainting",).to(device)
...
class Pix2Pix:
    def __init__(self, device):
        ...
        self.device = device
        self.pipe = StableDiffusionInstructPix2PixPipeline.from_pretrained(f"{pretrained_model_dir}/instruct-pix2pix", torch_dtype=torch.float16, safety_checker=None).to(device)
...
class T2I:
    def __init__(self, device):
        ...
        self.device = device
        self.pipe = StableDiffusionPipeline.from_pretrained(f"{pretrained_model_dir}/stable-diffusion-v1-5", torch_dtype=torch.float16)
        self.text_refine_tokenizer = AutoTokenizer.from_pretrained(f"{pretrained_model_dir}/MagicPrompt-Stable-Diffusion")
        self.text_refine_model = AutoModelForCausalLM.from_pretrained(f"{pretrained_model_dir}/MagicPrompt-Stable-Diffusion")
...
class ImageCaptioning:
    def __init__(self, device):
        ...
        self.model = BlipForConditionalGeneration.from_pretrained(f"{pretrained_model_dir}/blip-image-captioning-base").to(self.device)
...
class BLIPVQA:
    def __init__(self, device):
        ...
        self.model = BlipForQuestionAnswering.from_pretrained(f"{pretrained_model_dir}/blip-vqa-base").to(self.device)
...
if __name__ == '__main__':
    ...
    total_parser.add_argument("--hf_models_path", default="./hf_models_path",
                            type=str, help="huggingface下载好的模型路径")
    args = total_parser.parse_args()

    pretrained_model_dir = args.hf_models_path
